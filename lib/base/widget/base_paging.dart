...
abstract class BasePagingWidget extends StatefulWidget {
  const BasePagingWidget({Key key}) : super(key: key);
}

abstract class BasePagingState<W extends BasePagingWidget> extends State<W> {
  ...
  Widget bodyContentView(BuildContext context, dynamic state);
  ...
  Widget handleState(BuildContext context, BaseState state) {
    ...
    if (state is LoadMoreSuccessState) {
      ...
      return bodyContentView(context, state);
    }
